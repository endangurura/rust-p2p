        ...
        match read_local_recipes().await {
            Ok(recipes) => {
                ...
                if let Err(e) = sender.send(resp) {
                    error!("error sending response vida channel, {}", e);
                }
            }
...
async fn main() {
    ...
    loop {
        let evt = {
            tokio::select! {
                line = stdin.next_line() => Some(EventType::Input(line.expect("can get line").expect("can read lone from stdin"))),
                event = swarm.next() => {
